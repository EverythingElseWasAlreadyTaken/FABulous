...
It takes the path to a `.bin` bitstream file generated by `compile_design` and
runs the simulation via the Taskfile; the testbench reads the binary directly
and shifts in exactly as many bytes as the bitstream contains. The design name
is inferred from the bitstream filename unless overridden with `-d`.
