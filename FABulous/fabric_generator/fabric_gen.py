...
class FabricGenerator:
    ...
    def genIOBel(self, gen_ios: list[Gen_IO], bel_name: str) -> Bel | None:
        """
        ...
        if configBits > 0:
            self.writer.addParameterStart(indentLevel=1)
            self.writer.addParameter(
                "NoConfigBits", "integer", configBits, indentLevel=2
            )
            self.writer.addParameterEnd(indentLevel=1)
        self.writer.addPortStart(indentLevel=1)
        ...
        if configBits > 0:
            if self.writer.__class__ == VHDLWriter:
                ...
                self.writer.addPortVector(
                    "ConfigBits", IO.INPUT, configBits, indentLevel=2
                )
            else:  #  Verilog
                self.writer.addPortVector(
            ...
            if gio.clocked:
                ...
                if gio.clockedMux:
                    for i in range(gio.pins):
                        ...
                        if self.fabric.multiplexerStyle == MultiplexerStyle.CUSTOM:
                            portsPairs = [("A0",source),("A1", reg), ("S", f"ConfigBits[{i}]"), ("X",sink)]
                            self.writer.addInstantiation(
                                compName="cus_mux21",
