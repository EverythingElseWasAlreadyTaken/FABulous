...
class FABulous_API:
    ...
    def genIOBelForTile(self, tile_name: str) -> list[Bel]:
        ...
        if self.fabric.tileDic.get(tile_name):
            ...
            logger.warning(f"Tile {tile_name} is not used in fabric, but defined in fabric.csv.")
            self.fabric.unusedTileDic[tile_name].bels += bels
        else:
            logger.error(f"Tile {tile_name} is not defined in fabric, please add to fabric.csv.")
            raise ValueError
